...
The ZK Coprocessor exposes the data of the contract in a very well known form: an SQL table.
More specifically, the ZK Coprocessor keeps historical valuess for each block. **In a nutshell, it is a
time series database that keeps appending the values of all variables tracked for each block.**

Let's take the previous example with the two variables `price` and `balances`. The ZK Coprocessor
exposes these two variables as two separated tables:

### Table for single variables

ZK Coprocessor exposes a single table for all the different "single" variables (uint256, etc),
...
ZK Coprocessor exposes a single table _per_ "aggregate" variable such as a mapping and arrays.
