...
**State Committee Contracts** - The Lagrange service and Lagrange committee smart contract is deployed on the Ethereum mainnet. The smart contract manages epoch updates, voting power updates and calculates the Merkle tree.  A Merkle tree is computed by hashing individual data blocks to form leaf nodes, then iteratively hashing pairs of nodes to form parent nodes until a single hash, the Merkle root, is obtained. This root summarizes the data, allowing efficient and secure verification of the data’s integrity with minimal information.
